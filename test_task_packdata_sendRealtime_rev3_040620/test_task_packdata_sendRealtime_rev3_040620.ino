/*-------------------------Code 30/05/2020 -------------------------*/
// This Version Can check DataBase Sever
...
static void task_packDataSend(void* Param){
 for(;;){
...
void setup(){
  ...
  delay(100);
   client.setServer(mqtt_server,mqttPort); //For debug mqtt connect if COMMENT
   client.setCallback(IOTCallback);
    
    TaskHandle_t CheckWiFi_and_Mqtt;
    TaskHandle_t PackData_sendMqtt;
   
    xTaskCreatePinnedToCore(
             task_CheckWiFi_and_Mqtt, 
             "CheckWiFi_and_Mqtt",   
             5000,     
             NULL,      
             1,        
